...
router.post('/', async (req, res) => {
  try {
    ...
    run(`INSERT INTO products (name, image, quantity, description, hsn_code, sell_price, wholesale_price, inward_price, serial_number, discount_percent, barcode, barcode_image, category_id, gst_rate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [data.name, data.image || '', data.quantity || 0, data.description || '', data.hsn_code || '', data.sell_price || 0, data.wholesale_price || 0, data.inward_price || 0, serial, data.discount_percent || 0, barcode, barcode_image, data.category_id || null, data.gst_rate || 18]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    run(`UPDATE products SET name=?, image=?, quantity=?, description=?, hsn_code=?, sell_price=?, wholesale_price=?, inward_price=?, discount_percent=?, barcode=?, barcode_image=?, category_id=?, gst_rate=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
      [data.name || existing.name, data.image || existing.image, data.quantity ?? existing.quantity, data.description || existing.description, data.hsn_code || existing.hsn_code, data.sell_price ?? existing.sell_price, data.wholesale_price ?? existing.wholesale_price, data.inward_price ?? existing.inward_price, data.discount_percent ?? existing.discount_percent, barcode, barcode_image, data.category_id ?? existing.category_id, data.gst_rate ?? existing.gst_rate ?? 18, req.params.id]);
